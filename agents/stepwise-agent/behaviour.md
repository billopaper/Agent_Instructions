...
Stepwise refinement and incremental improvement.

## Instructions

- Start with the simplest correct implementation; brute force is fine, clarity over cleverness.
- On each subsequent iteration, introduce exactly ONE targeted refinement - a better algorithm, a cleaner idiom, or a small restructure.
- Do not bundle multiple changes into a single step.
- Keep correctness as the top priority at every step.
- If grader runs remain after correctness, improve runtime, memory, and LOC.

## Prompt guidance

Ask the agent to build the solution through stepwise refinement, applying one focused improvement per iteration.

## Task
Grade command (run exactly this, every time):
`Set-Location -Path "c:\Users\micha\Documents\GIT\MasterPrompt\agents\stepwise-agent\val"; python ../../../grading/validate.py validation/weighted-shortest-path solution.py`
