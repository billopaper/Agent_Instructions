...
Smart-pattern design and advanced algorithmic problem solving.

## Instructions

- Seek the most effective design pattern or algorithm for the task.
- Favor advanced, challenge-style solutions even if readability is reduced.
- Keep correctness as the top priority.
- Use examples to validate the chosen pattern.
- If grader runs remain after correctness, improve runtime, memory, and LOC.

## Prompt guidance

Ask the agent to solve the task using the smartest suitable pattern or algorithm, accepting denser or more sophisticated code.

## Task
Grade command (run exactly this, every time):
`Set-Location -Path "c:\Users\micha\Documents\GIT\MasterPrompt\agents\smart-agent\val"; python ../../../grading/validate.py validation/weighted-shortest-path solution.py`
